...
const Calendar: React.FC<CalendarProps> = ({
  ...
  // 디버깅용 로그 - 캘린더 초기화 시 가용 병원 데이터 확인
  console.log('캘린더 가용 병원 데이터:', availableCounts);
  ...
  const handleDateClick = (date: Date) => {
    ...
    const dateStr = date.toISOString().split('T')[0];
    // 디버깅용 로그 - 날짜 선택 시 해당 날짜의 가용 병원 수 확인
    console.log('선택된 날짜 병원 정보:', { 
      date: dateStr, 
      hospitalCount: availableCounts[dateStr] 
    });
